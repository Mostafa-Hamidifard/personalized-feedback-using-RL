    ...
    class Brain():
        ...
        def _calculate_percieved_error(self,stimulus,angles):
            ...
            for idx in range(n):
                angle = angles[idx]
                offset = 0 #5*np.pi/18 
                vec = np.array([np.cos(angle+offset) , np.sin(angle+offset)])
                # print(f"{idx} , {vec[0]} , {vec[1]}")
                percieved_error = percieved_error + stimulus[idx] * vec
                
            
            # print(percieved_error)
